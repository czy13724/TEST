...
# 获取 Gist ID 的脚本路径
get_gist_id_script = os.path.join(os.getcwd(), ".github", "scripts", "gist", "get_gist_id.py")
...
def get_gist_id():
    try:
        gist_id = os.popen("python3 " + get_gist_id_script).read().strip()
        return gist_id
    except Exception as e:
        print(f"Error getting Gist ID: {e}")
        return None

# 创建一个 result 字典用于存放任务信息
result = {
    "name": "Levi Tasks Collection",
    "description": "For removal of infringement, please contact @PMLevibot. Telegram bot: https://t.me/PMLevibot",
    "task": []
}
...
if gist_id:
    ...
    for js_file in js_files:
        ...
        task_entry["config"] += f"{cron_expression} https://gist.githubusercontent.com/czy13724/{gist_id}/raw/main/javascript/{js_file}, tag={js_file[:-3]}, img-url=https://raw.githubusercontent.com/czy13724/TEST/main/image/{js_file[:-3]}.png, enabled=false"

        # 判断是否有配置文件，决定是否添加 addons 字段
        if conf_files:
            # 如果有配置文件，则添加 addons 字段
            conf_file = conf_files[0]  # 只取第一个配置文件，你的需求是一个脚本对应一个配置文件
            task_entry["addons"] = f"https://gist.githubusercontent.com/czy13724/{gist_id}/raw/main/conf/{conf_file}, tag={js_file[:-3]}"
    ...
    print("Error getting Gist ID. Aborting.")
