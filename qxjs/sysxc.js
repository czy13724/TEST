...
by-
@Levi
感谢樱花佬的脚本框架@sliverkiss 频道 https://t.me/sliverkiss

@Date:2024-02-15完成脚本编写
...
脚本默认 上午9点10分 执行，如需更改请自行修改cron表达式
===============================================
Surge、ShadowRocket、LanceX配置如下：

[Script]
书亦烧仙草签到 = type=cron, cronexp="10 9 * * *", script-path=https://raw.githubusercontent.com/czy13724/TEST/main/qxjs/sysxc.js

===============================================
Quantumult X配置如下：

[task_local]
10 9 * * * https://raw.githubusercontent.com/czy13724/TEST/main/qxjs/sysxc.js, tag=书亦烧仙草签到, img-url=https://raw.githubusercontent.com/czy13724/LeviIcons/main/leviicons/sysxc.png, enabled=true

[rewrite_local]
^https:\/\/scrm-prod\.shuyi\.org\.cn\/saas-gateway\/api\/agg-trade\/v1\/signIn\/querySignInRecord$ url script-request-body https://raw.githubusercontent.com/czy13724/TEST/main/qxjs/sysxc.js
...
【 签到脚本使用教程 】:
*************************
ck内容均可通过Boxjs进行管理sysxc_data。
单账号：
远程引用重写开启抓包并打开书亦小程序随意浏览，提示获取ck成功则可以使用该脚本。
2.关闭获取cookie脚本，防止产生不必要的mitm
3.运行签到脚本，查看日志，正常情况下会提示签到成功。
⚠️：如无法获取ck请开启抓包随意浏览，抓包https://scrm-prod.shuyi.org.cn...域名下的auth。打开boxjs->我的->数据查看器->在数据键输入sysxc_data,点击VIEW->在数据内容输入抓取到的auth，点击保存。
多账号：
1.抓包https://scrm-prod.shuyi.org.cn...域名下的auth
2.打开boxjs->我的->数据查看器->在数据键输入sysxc_data,点击VIEW->在数据内容输入抓取到的auth，点击保存。
3.若有多账号，用@分割，如auth1@auth2
4.运行签到脚本，查看日志，正常情况下会提示签到成功。
...
async signin() {
    try {
        ...
        if (result?.status === true) {
                $.log(`✅签到成功！`);
                $.signMsg = `✅签到成功(你当前共计获得${result?.extFields?.availablePoints}积分，积分有效期内尽快使用！)`;
            }
         else {
           // $.log(`❌签到失败！`);
            $.signMsg = `❌签到失败(你当前共计获得${result?.extFields?.availablePoints}积分，积分有效期内尽快使用！)`;
        }
    } catch (e) {
        console.log(e);
    }
...
async point() {
        try {
            ...
            if (result?.resultCode === 0) {
                    $.log(`✅查询成功！`);
                    $.pointMsg = `🌈当前积分:${result?.extFields?.availablePoints}个，积分有效期内尽快使用！`;
            } else {
                $.log(`❌查询失败！`);
                $.pointMsg = `❌查询失败`;
            }
...
async function getCookie() {
    if ($request && $request.method != 'OPTIONS') {
        const tokenValue = $request.headers['auth'] || $request.headers['authorization'] || $request.headers['Authorization'];
        if (tokenValue) {
