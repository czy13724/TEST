...
4.查询积分：
(方法一：
请开启抓包查找https://bbs-api.iqoo.com/api/v3/user?userId=该域名后的userId，打开boxjs->我的->数据查看器->在数据键输入iqooc_userId,点击VIEW->在数据内容输入抓取链接最后的userId，点击保存。
方法二：点击'我的'，查看消息上方的爱酷号，打开boxjs->我的->数据查看器->在数据键输入iqooc_userId,点击VIEW->在数据内容输入抓取链接最后的userId，点击保存。
多账号：
