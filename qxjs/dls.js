...
https://raw.githubusercontent.com/czy13724/TEST/main/surge/dls.sgmodule

本地-Script定时任务
[Script]
# （默认上午9点 执行，如需更改请自行修改cron表达式）
杜蕾斯签到 = type=cron, cronexp="0 9 * * *", script-path=https://raw.githubusercontent.com/czy13724/TEST/main/qxjs/dls.js

Qx
[rewrite_local]
^https:\/\/vip\.ixiliu\.cn\/mp\/sign\/applyV2$ url script-request-body https://raw.githubusercontent.com/czy13724/TEST/main/qxjs/dls.js
...
多账号：
1.抓包https://vip.ixiliu.cn域名下的Access-Token
...
async point() {
        try {
            ...
            if (result?.status === 200) {
                    $.log(`✅查询成功！`);
                    $.pointMsg = `✅积分:${result?.data?.user_points}个`;
            } else {
                $.log(`❌查询失败！`);
                $.pointMsg = `❌查询失败`;
            }
